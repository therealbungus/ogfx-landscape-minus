...
# Get a list of files which contribute to the actual build of the grf (if using a hg checkout)
MANIFEST ?= $(shell if [ "$(USED_VCS)" == "HG" ]; then echo "`hg st -ardmcn --exclude .devzone --exclude .hgtags --exclude .hgignore`"; fi)
